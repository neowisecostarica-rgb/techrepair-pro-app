...
export function sanitizeOrganization(organization) {
  ...
  const {
    id, name, legal_name, country, currency, plan, status, partner_id,
    telefono_negocio, logo_url, email, direccion_comercial, tipo_entidad,
    identificacion_fiscal, direccion_fiscal, public_base_url, garantia_config,
    saldo_caja_inicial, saldo_caja_actual, ultima_actualizacion_caja,
    inventario_config, marketing_spend, authorization_preset_version,
    custom_grants_enabled, provisioning_status, provisioned_at,
  } = organization;
  return {
    id, name, legal_name, country, currency, plan, status, partner_id,
    telefono_negocio, logo_url, email, direccion_comercial, tipo_entidad,

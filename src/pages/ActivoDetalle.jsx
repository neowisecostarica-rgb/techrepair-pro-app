...
function ActivoDetalleContent() {
  ...
  if (hasDispositionEvidence) decisionSignals.push({ icon: Recycle, label: t('activo.disposalEvidence','Existe evidencia de disposición'), detail: `${recycling.length} registro${recycling.length === 1 ? '' : 's'} de reciclaje/disposición en su historia. Revisar antes de volver a operar o reasignar.`, tone: 'slate' });
  if (canReadCommercial && totalCost > 0) decisionSignals.push({ icon: CircleDollarSign, label: t('assetDetail.economicContext','Contexto económico disponible'), detail: `${money(totalCost)} pagado registrado a través de intervenciones asociadas. Es contexto histórico, no una recomendación automática de reemplazo.`, tone: 'slate' });
  if (history.length > 0 && !repeatedAttention && !hasQualitySignal && !hasDispositionEvidence) decisionSignals.push({ icon: Activity, label: t('assetDetail.noExtraordinarySignals','Sin señales operativas extraordinarias'), detail: 'El historial visible no muestra recurrencia alta, no conformidades ni disposición registrada. Continuar seguimiento normal.', tone: 'emerald' });
